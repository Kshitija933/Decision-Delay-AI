...
def _cause_scores(row: dict, rng: np.random.Generator) -> dict:
    ...
    noise = lambda: float(rng.normal(0, 0.5))

    return {
        "Fear of Failure":
            pfl * 1.5 + (10 - se) * 0.9 + sl * 0.3 + noise(),
        "Overwhelm / Complexity":
            td  * 1.6 + dl  * 0.6 + (10 - gc) * 0.3 + noise(),
        "Lack of Immediate Reward":
            tr  * 1.5 + (10 - im) * 0.8 + (10 - ss) * 0.3 + noise(),
        "Past Failure Loop":
            pfl * 1.7 + sl  * 0.6 + (10 - hs) * 0.3 + noise(),
        "Perfectionism":
            (10 - gc) * 0.7 + td * 0.9 + sl * 0.7 + (10 - se) * 0.3 + noise(),
        "Decision Fatigue":
            dl  * 1.1 + sl  * 0.9 + (10 - ss) * 0.5 + (10 - hs) * 0.3 + noise(),
    }
