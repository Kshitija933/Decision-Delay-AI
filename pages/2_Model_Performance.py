...
# ── Dark-theme palette ──
COBALT  = "#38BDF8"   # sky blue — visible on dark
CORAL    = "#FB7185"   # rose
VIOLET  = "#A78BFA"   # violet
AMBER   = "#FCD34D"   # amber/yellow
MINT    = "#34D399"   # mint green
SKY     = "#67E8F9"   # cyan
TEXT    = "#E2E8F0"   # near-white for all text
MUTED   = "#94A3B8"   # light grey for secondary labels
...
<div style="margin-bottom:1.5rem;">
  <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.3rem;">
    ...
    <h2 style="margin:0;font-family:'Plus Jakarta Sans',sans-serif;font-size:1.9rem;
               font-weight:800;color:#E2E8F0;letter-spacing:-0.02em;">Model Performance</h2>
  </div>
  <div style="height:3px;width:60px;background:linear-gradient(90deg,#38BDF8,#A78BFA);
              border-radius:2px;margin-bottom:0.5rem;"></div>
  <p style="color:#94A3B8;font-size:0.96rem;margin:0;font-weight:500;">DecisionDelayNet</p>
</div>
...
if model_exists:
    ...
    achieved = acc >= 90.0
    color    = "#34D399" if achieved else "#38BDF8"
    ...
    <div class="da-card da-card-{'mint' if achieved else 'accent'}">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;">
        <div>
          <div style="font-size:0.72rem;color:#94A3B8;font-weight:800;letter-spacing:0.1em;">
          ...
          <div style="color:#CBD5E1;font-size:0.82rem;margin-top:0.2rem;">
        ...
        <div style="text-align:right;">
          ...
          <div style="font-size:0.72rem;color:#94A3B8;margin-top:0.1rem;">
            Best Val: <span style="color:#FCD34D;font-weight:700;">{meta.get('best_val_acc',0)*100:.2f}%</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="da-card da-card-amber">
      <div style="font-weight:800;color:#FCD34D;font-size:1rem;">⚠️ Model Not Trained Yet</div>
      <p style="color:#CBD5E1;font-size:0.88rem;margin:0.5rem 0 0;line-height:1.8;">
        Run the full pipeline in order:<br>
        <code style="background:rgba(252,211,77,0.12);padding:0.1rem 0.5rem;border-radius:4px;
                     color:#FCD34D;font-weight:700;">python data/generate_raw_data.py</code><br>
        <code style="background:rgba(252,211,77,0.12);padding:0.1rem 0.5rem;border-radius:4px;
                     color:#FCD34D;font-weight:700;">python data/preprocessing.py</code><br>
        <code style="background:rgba(252,211,77,0.12);padding:0.1rem 0.5rem;border-radius:4px;
                     color:#FCD34D;font-weight:700;">python training/train.py</code>
      </p>
    </div>
    """, unsafe_allow_html=True)
...
with c1:
    ...
    fig.add_trace(go.Scatter(x=ep, y=val_loss,   name="Val",   line=dict(color=CORAL,width=2.5,dash="dot")))
    # warmup shading
...
fig4 = go.Figure(go.Heatmap(
    ...
    colorscale=[[0,"#0D1526"],[0.35,"#1B3A6B"],[1,"#38BDF8"]],
    text=cm.astype(int), texttemplate="%{text}",
    textfont=dict(size=12,color="#FFFFFF"), showscale=False,
))
...
for h,name in zip(hc,met.keys()):
    h.markdown(f'<div style="font-size:0.72rem;font-weight:800;color:#67E8F9;'
...
for _,row in df_m.iterrows():
    ...
    rc[0].markdown(f'<div style="font-size:0.86rem;color:#E2E8F0;font-weight:600;">'
...
st.markdown("""
<div style="border-top:1.5px solid rgba(110,231,247,0.15);margin-top:0.75rem;
            padding-top:0.75rem;display:flex;justify-content:space-between;font-size:0.88rem;">
  <span style="color:#CBD5E1;font-weight:600;">Macro Avg F1</span>
  <span style="color:#34D399;font-weight:900;">0.909</span>
...
colors_b = ["#1E3A5F"]*6 + ["#2D6A9E", MINT]
fig5 = go.Figure(go.Bar(
    x=bench["Model"], y=bench["Accuracy"],
    marker=dict(color=colors_b, opacity=0.95, line=dict(width=0.5,color="rgba(255,255,255,0.15)")),
