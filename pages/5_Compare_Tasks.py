...
# Dark panel backgrounds for Task A / Task B
PANEL_A = "rgba(56,189,248,0.06)"    # subtle cyan tint
PANEL_B = "rgba(251,113,133,0.06)"   # subtle rose tint
...
/* Make all labels light inside panels */
.task-panel-a label,
.task-panel-b label,
...
<div style="margin-bottom:1.5rem;">
  <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.3rem;">
    ...
    <h2 style="margin:0;font-family:'Plus Jakarta Sans',sans-serif;font-size:1.9rem;
               font-weight:800;color:#E2E8F0;letter-spacing:-0.02em;">Compare Tasks</h2>
  </div>
  <div style="height:3px;width:60px;background:linear-gradient(90deg,#38BDF8,#A78BFA);
              border-radius:2px;margin-bottom:0.5rem;"></div>
  <p style="color:#94A3B8;font-size:0.96rem;margin:0;font-weight:500;">Analyze two tasks side-by-side to see which has a higher delay risk</p>
</div>
""", unsafe_allow_html=True)
...
with col_mid:
    ...
    <div style="display:flex;align-items:center;justify-content:center;height:100%;
                min-height:200px;font-size:1.5rem;font-weight:800;color:#94A3B8;">vs</div>
    """, unsafe_allow_html=True)
...
if run:
    ...
    <div class="da-card" style="border-left:4px solid {winner_color};text-align:center;padding:1.25rem;">
      <div style="font-size:0.72rem;color:#67E8F9;font-weight:700;margin-bottom:0.3rem;letter-spacing:0.1em;">RECOMMENDATION</div>
      <div style="font-size:1.1rem;color:#E2E8F0;font-weight:600;">{winner_msg}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for col, result, task, color, panel_bg, label in [
        (res_a_col, result_a, task_a, COBALT, "rgba(56,189,248,0.06)",  "Task A"),
        (res_b_col, result_b, task_b, CORAL,   "rgba(251,113,133,0.06)", "Task B"),
    ]:
        with col:
            ...
            sev_color = MINT if sev < 0.35 else AMBER if sev < 0.65 else CORAL

            st.markdown(f"""
            <div style="background:{panel_bg};border:1.5px solid {color}44;border-left:4px solid {color};
              ...
              <div style="display:flex;justify-content:space-between;align-items:center;margin:0.4rem 0;">
                ...
                <div style="text-align:right;">
                  ...
                  <div style="font-size:0.72rem;color:#94A3B8;font-weight:600;">Delay</div>
                </div>
              </div>
              <div style="font-weight:700;font-size:0.92rem;color:#E2E8F0;margin-bottom:0.2rem;">{cause}</div>
              <div style="color:#94A3B8;font-size:0.8rem;font-weight:500;">Confidence: {conf*100:.1f}%</div>
            </div>
            """, unsafe_allow_html=True)

            st.markdown(f"""
            <div class="nudge-box" style="border-left:3px solid {sev_color}; background:rgba(255,255,255,0.03);">
              <div style="font-size:0.65rem; color:{MUTED}; font-weight:700; text-transform:uppercase; margin-bottom:0.3rem;">Strategic Action</div>
              <div class="nudge-text" style="font-size:0.92rem; color:#FFFFFF;">
                <strong>{nudge['action_plan']['micro_win']}</strong>
              </div>
              <div style="font-size:0.78rem; color:{color}; margin-top:0.4rem; font-style:italic;">
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        fillcolor="rgba(56,189,248,0.12)", line=dict(color=COBALT, width=2.5),
    ))
    fig.add_trace(go.Scatterpolar(
        r=vb, theta=cats, name=task_b["name"], fill="toself",
        fillcolor="rgba(251,113,133,0.12)", line=dict(color=CORAL, width=2.5),
    ))
    fig.update_layout(
        **_layout, height=360,
        polar=dict(
            ...
            angularaxis=dict(tickfont=dict(color=TEXT, size=10),
                             gridcolor="rgba(110,231,247,0.07)"),
        ),
    ...
    fig2.add_trace(go.Bar(
        ...
        marker=dict(color=CORAL, opacity=0.85),
    ))
    fig2.update_layout(
