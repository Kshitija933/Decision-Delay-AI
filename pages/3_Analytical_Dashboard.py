...
from utils.visualizer import (
    gauge_chart, MINT, AMBER, CORAL, VIOLET, COBALT, SKY, TEXT, MUTED
)

# ── Dark-theme Palette ──
GRID   = "rgba(110,231,247,0.07)"
GRID   = "rgba(110,231,247,0.07)"
CAUSE_COLORS = {
    "Fear of Failure":          CORAL,
    "Overwhelm / Complexity":   AMBER,
    "Lack of Immediate Reward": COBALT,
    "Past Failure Loop":        VIOLET,
    "Perfectionism":            "#FB923C",
...
<div style="margin-bottom:1.5rem;">
  <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.3rem;">
    ...
    <h2 style="margin:0;font-family:'Plus Jakarta Sans',sans-serif;font-size:1.9rem;
               font-weight:800;color:#E2E8F0;letter-spacing:-0.02em;">Analytical Dashboard</h2>
  </div>
  <div style="height:3px;width:60px;background:linear-gradient(90deg,#38BDF8,#A78BFA);
              border-radius:2px;margin-bottom:0.5rem;"></div>
  <p style="color:#94A3B8;font-size:0.96rem;margin:0;font-weight:500;">
...
if not history:
    ...
    <div class="da-card" style="text-align:center;padding:3rem 2rem;">
      ...
      <div style="font-size:1.2rem;font-weight:800;color:#E2E8F0;margin-bottom:0.5rem;">
        No Analyses Yet
      </div>
      <div style="color:#94A3B8;font-size:0.95rem;margin-bottom:1.5rem;line-height:1.7;">
        Run a <strong style="color:#E2E8F0;">Self Assessment</strong> first — your results will appear
    ...
    st.markdown("""
    <div style="font-size:0.72rem;font-weight:800;color:#67E8F9;letter-spacing:0.14em;
...
# ─────────────────────────────────────────────────────────────
if is_demo:
    st.markdown('<div style="background:rgba(252,211,77,0.10);border:1px solid rgba(252,211,77,0.30);'
                'border-radius:10px;padding:0.6rem 1rem;margin-bottom:1rem;font-size:0.85rem;color:#FCD34D;">'
...
sev_color = MINT if avg_sev < 0.35 else AMBER if avg_sev < 0.65 else CORAL
...
for col, label, val, color in [
    ...
    (k2, "Top Delay Cause", top_cause[:16], CORAL),
    (k3, "Avg Severity",    f"{avg_sev*100:.1f}%<br><span style='font-size:0.75rem;color:{CORAL if (avg_sev*100-50)<0 else MINT};'>{'▲' if (avg_sev*100-50)>=0 else '▼'} {(avg_sev*100-50):.1f}</span>", sev_color),
    (k4, "Avg Confidence",  f"{avg_conf*100:.1f}%", VIOLET),
    (k5, "Severity Trend",  trend, MINT if "↓" in trend else AMBER),
]:
    with col:
        st.markdown(f"""
        <div class="da-card" style="text-align:center;padding:0.9rem 0.5rem;">
          <div style="font-size:0.65rem;color:#94A3B8;font-weight:800;
...
st.markdown('<hr style="border-color:rgba(110,231,247,0.10);margin:0.5rem 0 1rem;">', unsafe_allow_html=True)

# ─────────────────────────────────────────────────────────────
# ROW 1 — Severity Timeline + Cause Frequency Bar
# ─────────────────────────────────────────────────────────────
st.markdown('<div style="font-size:0.72rem;font-weight:800;color:#67E8F9;letter-spacing:0.14em;'
...
with r1a:
    ...
    fig1.update_layout(**_L, height=310,
        ...
        xaxis=dict(title="Analysis #", showgrid=False, tickfont=dict(size=10, color=MUTED),
                   linecolor="rgba(110,231,247,0.12)"),
        yaxis=dict(range=[0,1.05], showgrid=True, gridcolor="rgba(110,231,247,0.08)",
                   tickfont=dict(size=10, color=MUTED), linecolor="rgba(110,231,247,0.12)"),
        legend=dict(font=dict(size=10, color=TEXT), orientation="h", y=1.12),
    )
    # Use a random UUID key to ensure 'active' refreshing and animation
    import uuid
    chart_key = str(uuid.uuid4())
    fig1.update_layout(plot_bgcolor="#0D1627")
    st.plotly_chart(fig1, use_container_width=True, config={"displayModeBar": False}, key=f"db_timeline_{chart_key}")
...
with r1b:
    ...
    st.plotly_chart(fig2, use_container_width=True, config={"displayModeBar": False}, key=f"db_causes_bar_{chart_key}")
...
with r2a:
    fig3 = gauge_chart(avg_sev, title="Avg Severity")
    st.plotly_chart(fig3, use_container_width=True, config={"displayModeBar": False}, key=f"db_sev_gauge_{chart_key}")

with r2b:
    fig4 = gauge_chart(avg_conf, title="Avg Confidence", custom_color=COBALT)
    st.plotly_chart(fig4, use_container_width=True, config={"displayModeBar": False}, key=f"db_conf_gauge_{chart_key}")
...
with r2c:
    ...
    fig5 = go.Figure(go.Pie(
        ...
        hole=0.60,
        marker=dict(colors=[COBALT, VIOLET, CORAL],
    ...
    st.plotly_chart(fig5, use_container_width=True, config={"displayModeBar": False}, key=f"db_domain_donut_{chart_key}")

# ─────────────────────────────────────────────────────────────
# ROW 3 — Input Score Radar
# ─────────────────────────────────────────────────────────────
st.markdown('<hr style="border-color:rgba(110,231,247,0.10);margin:0.5rem 0 1rem;">', unsafe_allow_html=True)
st.markdown('<div style="font-size:0.72rem;font-weight:800;color:#67E8F9;letter-spacing:0.14em;'
...
with r3a:
    ...
    fig6.update_layout(**_L, height=320,
        ...
        polar=dict(
            bgcolor="rgba(13,26,62,0.6)",
            radialaxis=dict(visible=True, range=[0,1],
                            tickfont=dict(size=8, color=MUTED),
                            gridcolor="rgba(110,231,247,0.10)"),
            angularaxis=dict(tickfont=dict(color=TEXT, size=9)),
        ),
        legend=dict(font=dict(size=9, color=TEXT), x=0.75, y=1.1),
    )
    st.plotly_chart(fig6, use_container_width=True, config={"displayModeBar": False}, key="db_input_radar")
...
with r3b:
    ...
    bar_c = [CORAL if (l in ["Task Difficulty","Past Failures","Stress Level"] and v > 6)
    ...
    fig7 = go.Figure(go.Bar(
        ...
        marker=dict(color=bar_c, opacity=0.88, line=dict(color="rgba(255,255,255,0.1)", width=0.5)),
    ...
    fig7.update_layout(**_L, height=320,
        ...
        yaxis=dict(range=[0, 12], showgrid=True, gridcolor=GRID, tickfont=dict(size=10, color=MUTED)),
        bargap=0.3,
    )
    st.plotly_chart(fig7, use_container_width=True, config={"displayModeBar": False}, key="db_input_bars")
...
if prob_rows:
    ...
    fig8 = go.Figure(go.Heatmap(
        ...
        colorscale=[[0,"#1E3A5F"],[0.4,"#2563A8"],[1,COBALT]],
    ...
    st.plotly_chart(fig8, use_container_width=True, config={"displayModeBar": False}, key="db_prob_heatmap")

# ─────────────────────────────────────────────────────────────
# ROW 5 — Severity Distribution Histogram + Scatter
# ─────────────────────────────────────────────────────────────
st.markdown('<hr style="border-color:rgba(110,231,247,0.10);margin:0.5rem 0 1rem;">', unsafe_allow_html=True)
st.markdown('<div style="font-size:0.72rem;font-weight:800;color:#67E8F9;letter-spacing:0.14em;'
...
with r5a:
    ...
    bar_cols = [MINT if m < 0.35 else AMBER if m < 0.65 else CORAL for m in mid]
    fig9 = go.Figure(go.Bar(
        x=mid, y=counts_h,
        marker=dict(color=bar_cols, opacity=0.88, line=dict(color="rgba(255,255,255,0.1)", width=0.5)),
    ...
    fig9.update_layout(**_L, height=270,
        ...
        xaxis=dict(title="Severity", showgrid=False, tickfont=dict(size=10, color=MUTED)),
        yaxis=dict(showgrid=True, gridcolor=GRID, tickfont=dict(size=10, color=MUTED)),
        bargap=0.08, showlegend=False,
    )
    st.plotly_chart(fig9, use_container_width=True, config={"displayModeBar": False}, key="db_sev_dist")
...
with r5b:
    ...
    st.plotly_chart(fig10, use_container_width=True, config={"displayModeBar": False}, key="db_sev_scatter")
